Shader "TARARO/quad_grid-wave"
{
    Properties
    {
        ...
        [Header(Cube)]
            ...
            _BoxSize("Box Size (Ratio)", Range(0.0, 1.0)) = 1
    }
    ...
    float gridwave(float3 p) {
        float tDef = _Time.y * _WaveSpeed;
        float3 q = p - _Position;
        float2 fq = floor(q.xz / _GridSize);
        q.xz = fmod(q.xz, _GridSize) - (_GridSize * 0.5) + _GridSize * step(q.xz, 0);
        float d = box(q, float3(_GridSize * 0.5 * _BoxSize, perlinNoise3(float3(fq.x, fq.y, tDef)) * _WaveHeight, _GridSize * 0.5 * _BoxSize));
        return d;
    }
